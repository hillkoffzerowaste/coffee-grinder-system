...
export function useSounds(){
  ...
  const enable=useCallback(async(preview=true)=>{
    try {
      ...
      if(audio.current!==context)return false;
      if(context.state!=="running"){setError("เบราว์เซอร์รอการยืนยัน กรุณากดเปิดเสียงเพื่อเริ่มงาน");return false;}
      allowed.current=true;setEnabled(true);setError("");if(preview)play("success");return true;
    }catch{setError("เปิดเสียงไม่ได้ กรุณาตรวจการอนุญาตเสียงของเบราว์เซอร์");}
  },[play]);
  useEffect(()=>{
    let active=true;
    queueMicrotask(()=>{if(active)void enable(false);});
    return()=>{active=false;allowed.current=false;const context=audio.current;audio.current=null;void context?.close().catch(()=>undefined);};
  },[enable]);
  return {enabled,soundError,play,enable};
}
