...
import { UiConfigControls } from "@/components/ui-config-controls";
...
export function AdminConsole({ profile }: { profile: Profile }) {
  ...
  return <div className="app-shell"><Topbar title="Admin Console" profile={profile} /><main id="main" tabIndex={-1} className="admin-layout">
    ...
    <div className="admin-main stack"><UiConfigControls /><AdminOrderControls /><h2>{config[entity].title}</h2>{error && <div className="notice error" role="alert">{error}</div>}{message && <div className="notice success" role="status">{message}</div>}
