...
export function PackingWorkspace({ profile }: { profile: Profile }) {
  ...
  return <div className="app-shell operational-shell"><Topbar title="ห้องแพ็ค" profile={profile} /><main id="main" tabIndex={-1} className="workspace packing-home">
    <section className="panel packing-barcode-home"><SoundControls sound={sound} onReady={()=>scanRef.current?.focus()} /><div className="row"><h2>บาร์โค้ดเบอร์บด</h2><Link className="button secondary" href="/packing/new">เปิดออเดอร์เอง</Link>{queuedCount>0&&<button className="button" onClick={()=>setQueueOpen(true)}>คิวรอรับ {queuedCount} ถุง</button>}</div><section className="barcode-drawer"><GrindBarcodes grinds={grinds} error={catalogError} retry={reloadCatalog} /></section><small>รอรับ {queuedCount} ถุง · เสียงเตือนดังซ้ำทุก 3 วินาที จนงานรอรับทั้งระบบเหลือ 0 ถุง</small></section>
