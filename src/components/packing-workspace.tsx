...
export function PackingWorkspace({profile,initialManual=false,uiConfig}:{profile:Profile;initialManual?:boolean;uiConfig?:UiConfig}){
 ...
 return <div className="app-shell operational-shell" data-density={uiConfig?.theme.density} data-button-size={uiConfig?.theme.buttonSize}><Topbar title="ห้องแพ็ค" profile={profile} uiConfig={uiConfig}/><main id="main" tabIndex={-1} className="workspace grid packing-layout">
  ...
  <aside className="panel packing-detail"><h2>{batchId?"กำลังบด":"งานที่เลือก"}</h2><div className="detail-content">
   ...
   {context&&<><strong>{orderNo(context)} · ชุดที่ {context.blend_group_no??"-"}</strong><span className="band-parts"><BlendBadge skuCount={skuCountIn(orderJobs,context)} mode={context.process_mode} /></span><div className="product-name">{context.product_name_snapshot}</div><div>{context.sku_snapshot} · <SizeTag grams={context.size_grams_snapshot} mode={context.process_mode} /></div><div className="notice">{context.process_mode==="WHOLE_BEAN"?"งานเมล็ด — ตรวจสินค้าและระบุจำนวนถุงเพื่อเริ่มงาน":"สแกนเบอร์บด จากนั้นระบุจำนวนถุงเพื่อเข้าสถานะกำลังบด"}</div><div>เบอร์ที่รอรับ: {[...new Set(orderJobs.filter(j=>sameWork(j,context)&&canStart(j)).map(j=>j.grind_value_snapshot).filter(Boolean))].join(", ")||"เมล็ด/ไม่มี"}</div></>}
