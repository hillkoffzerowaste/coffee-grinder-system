...
export function PackingWorkspace({profile,initialManual=false,uiConfig}:{profile:Profile;initialManual?:boolean;uiConfig?:UiConfig}){
 ...
 const visibleRows=[...new Map(jobs.map(j=>[j.grinding_batch_id??`${j.order_id}:${j.blend_group_no??j.id}`,j])).values()];
 ...
 return <div className="app-shell operational-shell" data-density={uiConfig?.theme.density} data-button-size={uiConfig?.theme.buttonSize}><Topbar title="ห้องแพ็ค" profile={profile} uiConfig={uiConfig}/><main id="main" tabIndex={-1} className="workspace grid packing-layout">
  <section ref={queueRef} className="panel packing-queue"><SoundControls sound={sound} onReady={refocus}/>
   ...
   <div className="data-table-wrap"><table className="data-table"><thead><tr><th>คิว</th><th>ชุด / สินค้า / ออเดอร์</th><th>ขนาด</th><th>วิธีเตรียม</th><th>สถานะ</th><th>จัดการ</th></tr></thead><tbody>{visibleRows.map(j=><tr key={j.grinding_batch_id??`${j.order_id}:${j.blend_group_no??j.id}`}><td>#{j.queue_seq}</td><td>ชุดที่ {j.blend_group_no??"-"}<br/>{j.product_name_snapshot}<br/><small>{j.sku_snapshot} · {orderNo(j)}</small></td><td><SizeTag grams={j.size_grams_snapshot} mode={j.process_mode} /></td><td>{j.process_mode==="WHOLE_BEAN"?"เมล็ด":`บดเบอร์ ${j.grind_value_snapshot}`}</td><td><span className={`status ${j.status==="QUEUED"?"warn":"info"}`}>{jobStatusLabels[j.status]}</span>{j.grinding_batch_id&&<small> · ชุดงาน</small>}</td><td><button className="button secondary" disabled={busy||!!pending||recoveryError} onClick={()=>void choose(j)}>เปิดงาน</button></td></tr>)}</tbody></table>{!jobs.length&&<div className="empty">ไม่มีงานในคิวนี้</div>}</div>
  ...
  <aside className="panel packing-detail"><h2>{batchId?"กำลังบด":"งานที่เลือก"}</h2><div className="detail-content">
   ...
   {context&&<><strong>{orderNo(context)} · ชุดที่ {context.blend_group_no??"-"}</strong><div className="product-name">{context.product_name_snapshot}</div><div>{context.sku_snapshot} · <SizeTag grams={context.size_grams_snapshot} mode={context.process_mode} /></div><div className="notice">{context.process_mode==="WHOLE_BEAN"?"งานเมล็ด — ตรวจสินค้าและระบุจำนวนถุงเพื่อเริ่มงาน":"สแกนเบอร์บด จากนั้นระบุจำนวนถุงเพื่อเข้าสถานะกำลังบด"}</div><div>เบอร์ที่รอรับ: {[...new Set(orderJobs.filter(j=>groupKey(j)===groupKey(context)&&canStart(j)).map(j=>j.grind_value_snapshot).filter(Boolean))].join(", ")||"เมล็ด/ไม่มี"}</div></>}
   ...
   {batchId&&<><strong>{batchJobs[0]?orderNo(batchJobs[0]):"กำลังโหลดชุดงาน..."} · ชุดที่ {batchJobs[0]?.blend_group_no??"-"} · {batchJobs.length} ถุง</strong>{batchJobs.map(j=><div className="notice" key={j.id}>{j.product_name_snapshot} · {j.sku_snapshot}<br/><SizeTag grams={j.size_grams_snapshot} mode={j.process_mode} /> · {j.process_mode==="WHOLE_BEAN"?"เมล็ด":`เบอร์ ${j.grind_value_snapshot}`} · {j.grinder_name_snapshot}</div>)}</>}
