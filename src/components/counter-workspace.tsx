  ...
  const content = <>
    <main id="main" tabIndex={-1} className="workspace grid counter-layout">
      <section className="panel counter-composer"><div ref={composerRef} className="composer-content stack">
        ...
        <div className="data-table-wrap"><table className="data-table"><thead><tr><th>สินค้า</th><th>ขนาด</th><th>วิธีเตรียม</th><th>ถุง</th><th>จัดการ</th></tr></thead><tbody>{lines.map((line) => <tr key={line.clientLineId}><td>ชุดที่ {groupsFor(lines).findIndex(group => group.groupId === line.blendGroupId) + 1} · {line.product.name}<br /><small>{line.product.sku}</small></td><td><SizeTag grams={line.product.size_grams} mode={line.mode} /></td><td>{line.mode === "WHOLE_BEAN" ? "เมล็ด" : `บดเบอร์ ${line.grind?.grind_value}`}</td><td>{line.quantity}</td><td><button className="button secondary" disabled={busy || awaitingRetry || quantityOpen || recoveryRequired} onClick={() => { if (operation.current || awaitingRetry || recoveryRequired || quantityActive.current) return; setProduct(line.product); setMode(line.mode ?? "GROUND"); setGrind(line.grind); setActiveGroup({id:line.blendGroupId ?? line.clientLineId,mode:line.mode ?? "GROUND",grind:line.grind}); setEditingId(line.clientLineId); if (line.mode === "WHOLE_BEAN") { quantityActive.current=true; setQuantityOpen(true); } else if (line.grind) openQuantity(line.grind, line.quantity); }}>แก้ไข</button> <button className="button secondary" disabled={busy || awaitingRetry || quantityOpen || recoveryRequired} onClick={() => { if (operation.current || awaitingRetry || recoveryRequired || quantityActive.current) return; setLines((current) => current.filter((item) => item.clientLineId !== line.clientLineId)); requestId.current = null; }}>ลบ</button></td></tr>)}</tbody></table>{!lines.length && <div className="empty">ยังไม่มีรายการ</div>}</div>
