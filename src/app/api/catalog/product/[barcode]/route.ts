...
export async function GET(_: Request, context: {params:Promise<{barcode:string}>}) {
 ...
 try {
 const [item]=await readRows(auth.profile.id,"select p.id,p.sku,p.name,p.size_grams,p.unit,b.barcode from coffee.product_barcodes b join coffee.products p on p.id=b.product_id where b.barcode=$1 and b.active and p.active",[barcode]);
