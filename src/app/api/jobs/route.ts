...
export async function GET(request:Request) {
 ...
 const [queue]=await readRows(auth.profile.id,"select coalesce(max(queue_seq),0)::text as latest from coffee.bags");
 return NextResponse.json({jobs,latestQueueSeq:queue.latest});} catch(error) { const e=databaseError(error); return NextResponse.json({error:e.message},{status:e.status}); }
}
