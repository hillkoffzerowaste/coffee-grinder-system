...
Next.js on Vercel; PostgreSQL on Neon (`neon-bronze-chair`). No Supabase runtime or browser database keys are used. Historical `supabase/migrations` documents the former schema only; **do not run it** for Neon.
...
The initial catalog has 139 whole-bean SKUs >=200g. Seven products have no barcode in the source; add their barcodes in Admin before scanning. Future products and alternate barcodes are managed in Admin. Add real grinder/operator names in Admin before starting jobs.
...
- Packing refreshes via HTTP polling; no Supabase Realtime dependency.
...
Old architecture documents in `docs/` may mention the superseded Supabase design. This README and `database/migrations` describe the current implementation.
