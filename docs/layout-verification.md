...
- Action: run `npm run test:layout` and inspect the generated screenshots. Check entire barcode cards (including captions), nonzero table height, long Thai names, edit/add controls, product → grind → quantity modal → batch grinding → completion, and scanner focus after actions. Unit tests alone cannot establish visibility.
...
## Mutation recovery regression checks

- Trigger: changing order/batch confirmation or recovery storage.
- Action: validate response identifiers before deleting recovery data. Test malformed success replies, lost replies, reload, and 401/403/429 responses; retries must preserve the original request ID and body. Verify competing quantity claims on an isolated database branch.
- Evidence: a reproducible manual-order response containing 36 hyphens passed the old batch-ID regex and deleted recovery data. The regression now rejects it. Concurrent database checks verify that only the available quantity starts and replay creates no additional bags/events.
- Scope: local. Status: validated. Reviewed: 2026-09-04.
