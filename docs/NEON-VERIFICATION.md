...
- Runtime uses `pg` with a Vercel-managed pooling lifecycle.
- Neon `coffee` schema initialized transactionally; pre-existing `neon_auth` untouched.
- Catalog source: 143 whole-bean products >=200g, including four Thai-labelled red-bag products. Bootstrap applies this catalog and the versioned migrations transactionally; credentials are ignored locally.
