...
try{
 ...
 await page.route('**/api/jobs',route=>route.fulfill({json:{jobs:[],latestQueueSeq:latest}}));
 ...
 for(const path of ['/counter','/packing/new','/packing']){
  ...
  if(path==='/packing'){
   const count=await page.evaluate(()=>window.toneFrequencies.length);latest='11';
   ...
   await page.waitForTimeout(5500);assert.equal(await page.evaluate(()=>window.toneFrequencies.length),count+3);
  }
 ...
 console.log('Browser verification passed; no operational data written. New-job sound plays once per detected update.');
