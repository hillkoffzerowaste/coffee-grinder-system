-- ชุด (blend group) คือป้ายงานของห้องแพ็ค ไม่ใช่สูตรกาแฟ หน้าร้านจึงต้องใส่หลายเบอร์บด
-- ในชุดเดียวได้ เช่น ชุดของลูกค้ารายเดียวที่ขอซองแดงเบอร์ 6 กับซองเขียวเบอร์ 8
-- เงื่อนไข "หนึ่งชุดหนึ่งเบอร์" จึงถูกถอดออก ส่วนวิธีเตรียมยังบังคับให้เป็นแบบเดียวทั้งชุด
-- เพราะงานเมล็ดกับงานบดเดินคนละเครื่องคนละคิว
-- start_scan_batch เลือกถุงด้วย (order, product_barcode, grind_id, blend_group_no) อยู่แล้ว
-- การบดทีละเบอร์ในชุดเดียวจึงไม่ต้องแก้

create or replace function coffee.create_order(p_client_request_id uuid, p_source text, p_lines jsonb, p_note text)
...
declare
  ...
  line jsonb; group_key text; group_mode text; group_no integer; q integer; total integer := 0; bag_index integer := 0; n integer;
begin
